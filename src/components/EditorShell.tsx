        ...
        {isMobile ? (
          ...
          <div className="shrink-0 ml-1 relative" ref={exportMenuRef}>
            ...
            {showExportMenu && (
              ...
              >
                {(['MD', 'TXT', 'HTML', 'DOCX', 'PDF', 'JSON'] as const).map((fmt) => (
                  <button
                    key={fmt}
                    onClick={() => {
                      const handler = fmt === 'MD' ? exportMd : fmt === 'TXT' ? exportTxt : fmt === 'HTML' ? exportHtml : fmt === 'DOCX' ? exportDocx : fmt === 'PDF' ? exportPdf : exportJson;
                      handler();
                      setShowExportMenu(false);
                    }}
